...
def Decode(arguments): #Performs the decoding
    ...
    display = ProgressDisplay(arguments.buffer, False, fin.Filename, fout.Filename).Begin()
    try:
...
def Encode(arguments): #Performs the encoding
    ...
    display = ProgressDisplay(arguments.buffer, True, fin.Filename, fout.Filename).Begin()
    try:
